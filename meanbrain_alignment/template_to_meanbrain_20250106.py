...
# Input parameters
meanbrain_tag = '20241028_bifrost_ch1_output'
...
print('Meanbrain shape:', meanbrain.shape)
print('Template shape:', template.shape)
print('Glom mask shape:', glom_mask.shape)

n_meanbrain_z = meanbrain.shape[2]
...
#     ax[z].imshow(meanbrain[:, :, z*3].numpy().T)

_ = plot_alignment(reg_list, meanbrain, template, glom_mask, 
...
reg = ants.registration(fixed=ants.n4_bias_field_correction(meanbrain),
...
_ = plot_MI(reg_list, meanbrain, template)
fh,_ = plot_alignment(reg_list, meanbrain, template, glom_mask, 
                    disp_z_locs, disp_x_bounds, disp_y_bounds)
...
template_warped = ants.apply_transforms(fixed=meanbrain,
                                        moving=template,
...
merged = ants.merge_channels([meanbrain, template_warped, glom_mask_warped])
